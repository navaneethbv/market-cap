...
function getMarketMoveBadgeClass(marketMove: number): string {
  if (marketMove > 0) {
  ...
  return "bg-muted border-muted-foreground/10 text-muted-foreground";
}

function getProjectedMoveTextClass(projectedMove: number): string {
  if (projectedMove > 0) return "text-emerald-600 dark:text-emerald-400";
  if (projectedMove < 0) return "text-red-600 dark:text-red-400";
  return "text-foreground";
}

export default function VolatilitySimulatorPage({ params }: Readonly<PageProps>) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/stock/${upperSymbol}/beta`)
      .then((res) => {
        if (!res.ok) throw new Error("Failed to load asset beta data");
        return res.json();
      })
      .then((data) => {
        if (!active) return;
              ...
              <div className="flex justify-between items-center text-sm font-bold">
                ...
                <span
                  className={cn(
                    ...
                    getMarketMoveBadgeClass(marketMove)
                  )}
                >
                  {marketMove > 0 ? "+" : ""}
                  {marketMove}%
            ...
            <div className="p-5 rounded-xl border bg-card shadow-sm space-y-1 text-center">
              ...
              <div
                className={cn(
                  ...
                  getProjectedMoveTextClass(projectedMove)
                )}
              >
                {projectedMove > 0 ? "+" : ""}
