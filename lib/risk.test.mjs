...
test("calculateWeightedBeta aggregates beta based on asset weights", () => {
  ...
  // No assets means no measurable beta; do not fabricate a market-like 1.0.
  assert.equal(calculateWeightedBeta([]), null);
  assert.equal(calculateWeightedBeta([{ symbol: "X", value: 0, beta: 1.5 }]), null);
});
...
test("calculateHHI handles empty assets list", () => {
  assert.equal(calculateHHI([]), 0);
});
