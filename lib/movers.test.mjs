...
test("buildMoverRows handles non-Error rejection reasons", () => {
  const rows = buildMoverRows(["MSFT"], [
    { status: "rejected", reason: "custom rejection string" },
  ]);
  assert.equal(rows[0].error, "Quote unavailable");
});

test("calculateMoversSummary handles zero priced mover rows", () => {
  const summary = calculateMoversSummary([
    { symbol: "MSFT", quote: null, error: "error", direction: "unavailable" },
  ]);
