...
import { getStripe, getOrCreateProPriceId, _resetStripeCache } from "./stripe.ts";
...
test("getStripe throws when STRIPE_SECRET_KEY is not set", () => {
  _resetStripeCache();
  const originalKey = process.env.STRIPE_SECRET_KEY;
...
test("getOrCreateProPriceId returns existing price ID if found in Stripe", async () => {
  _resetStripeCache();
  const originalKey = process.env.STRIPE_SECRET_KEY;
  process.env.STRIPE_SECRET_KEY = "sk_test_mock_123";
  try {
    const stripe = getStripe();
    stripe.prices.list = async () => {
      return { data: [{ id: "price_existing_456" }] };
    };
    const priceId = await getOrCreateProPriceId();
    assert.equal(priceId, "price_existing_456");
  } finally {
    process.env.STRIPE_SECRET_KEY = originalKey;
  }
});

test("createCustomerPortalSession calls Stripe billingPortal sessions create", async () => {
  _resetStripeCache();
  const originalKey = process.env.STRIPE_SECRET_KEY;
  process.env.STRIPE_SECRET_KEY = "sk_test_mock_123";
  try {
    const { createCustomerPortalSession } = await import("./stripe.ts");
    const stripe = getStripe();
    ...
    stripe.billingPortal = {
      sessions: {
        create: async (params) => {
          ...
          assert.equal(params.return_url, "http://localhost:3000/pricing");
    ...
    assert.equal(url, "https://billing.stripe.com/session_test");
    assert.ok(portalCreated);
  } finally {
    process.env.STRIPE_SECRET_KEY = originalKey;
  }
});
