...
test("paused and malformed alerts never generate notifications", () => {
  const base = { symbol: "AAPL", target_price: 100, direction: "above", active: true };
  for (const alert of [{ ...base, active: false }, { ...base, target_price: Infinity }, { ...base, target_price: NaN }, { ...base, direction: "invalid" }]) {
    assert.deepEqual(evaluateAlertNotifications([alert], { AAPL: { price: 200 } }), { notifications: [], triggeredCount: 0 });
  }
  for (const price of [NaN, Infinity, -1, 0]) {
    assert.equal(evaluateAlertNotifications([base], { AAPL: { price } }).notifications.length, 0);
  }
});
