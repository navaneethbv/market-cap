...
test("subscriptionPeriodEnd and subscriptionHasPrice handle missing items/data", () => {
  const subWithoutItems = {
    id: "sub_123",
    status: "active",
    current_period_end: undefined, // trigger fallback to items
  };

  const state = deriveBillingState({
    customer: customerFor(),
    subscriptions: [subWithoutItems],
    userId: USER,
    proPriceId: PRICE,
  });

  // Since it has no items or prices, it shouldn't match the pro price
  assert.deepEqual(state, FREE_BILLING_STATE);
});

test("subscriptionPeriodEnd handles items without numeric period end", () => {
  const subWithNonNumericEnds = {
    id: "sub_123",
    status: "active",
    current_period_end: undefined,
    items: {
      data: [
        { price: { id: PRICE }, current_period_end: null },
      ],
    },
  };
  ...
  const state = deriveBillingState({
    customer: customerFor(),
    subscriptions: [subWithNonNumericEnds],
    userId: USER,
    proPriceId: PRICE,
  });

  assert.equal(state.isPro, true);
  assert.equal(state.currentPeriodEnd, null);
});

test("deriveBillingState handles multiple subscriptions with some null period ends", () => {
  const state = deriveBillingState({
    customer: customerFor(),
    subscriptions: [
      proSub({ id: "sub_null", current_period_end: null }),
      proSub({ id: "sub_numeric", current_period_end: 1800000000 }),
    ],
    userId: USER,
    proPriceId: PRICE,
  });

  assert.equal(state.subscriptionId, "sub_numeric");
  assert.equal(state.currentPeriodEnd, "2027-01-15T08:00:00.000Z");
});
