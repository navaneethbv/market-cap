...
test("calculateIncomeSummary excludes holdings without a current price", () => {
  const summary = calculateIncomeSummary([
    {
      symbol: "AAPL",
      shares: 10,
  ...
  assert.equal(summary.annualIncome, 12);
  assert.equal(summary.totalMarketValue, 600);
  assert.equal(summary.portfolioYield, 2);
  assert.equal(summary.yieldOnCost, 12 / 500 * 100);
});
