...
test("computeStockDimensions computes valid scores between 0 and 100", () => {
  ...
  assert.ok(scores.valuation >= 0 && scores.valuation <= 100);
  assert.ok(scores.profitability >= 0 && scores.profitability <= 100);
  assert.ok(scores.stability >= 0 && scores.stability <= 100);
  assert.ok(scores.momentum >= 0 && scores.momentum <= 100);
  assert.ok(scores.growth >= 0 && scores.growth <= 100);
});
