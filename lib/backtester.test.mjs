...
test("runBacktest handles SMA Crossover strategy golden/death cross signals", () => {
  ...
  // Sell log records the actual shares sold
  assert.ok(result.trades[1].shares > 0);
  assert.equal(result.trades[1].shares, result.trades[0].shares);
  assert.equal(
    result.trades[1].value,
    result.trades[1].shares * result.trades[1].price
  );
...
test("normalizeBacktestOptions throws on non-integer floating point periods", () => {
  assert.throws(
    () =>
      normalizeBacktestOptions({
        strategy: "sma_crossover",
  ...
  assert.throws(
    ...
    /invalid long SMA period/
  );
});

test("runBacktest handles candles count shorter than indicator periods", () => {
  const candles = [
    { time: "2026-06-01", open: 100, high: 100, low: 100, close: 100, volume: 100 },
    { time: "2026-06-02", open: 100, high: 100, low: 100, close: 100, volume: 100 },
  ];
  ...
  // Short candles list should run backtest starting at index 0 as fallback
  assert.equal(result.points.length, 2);
  assert.equal(result.trades.length, 0);
  assert.equal(result.winRate, 0);
});

test("runBacktest returns zero winRate when no trades are made", () => {
  const candles = [
    { time: "2026-06-01", open: 100, high: 100, low: 100, close: 100, volume: 100 },
    { time: "2026-06-02", open: 100, high: 100, low: 100, close: 100, volume: 100 },
    { time: "2026-06-03", open: 100, high: 100, low: 100, close: 100, volume: 100 },
