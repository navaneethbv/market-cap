...
test("buildComparisonRows handles string rejection reasons", () => {
  const rows = buildComparisonRows(["AAPL"], [
    { status: "rejected", reason: "custom rate limit reason" },
  ]);
  assert.equal(rows[0].error, "Quote unavailable");
});

test("calculateComparisonSummary handles empty priced rows", () => {
  const summary = calculateComparisonSummary([
    { symbol: "AAPL", quote: null, rank: null, error: "error" },
  ]);
