import assert from "node:assert/strict";
import { test } from "node:test";
import {
  filterScreenerStocks,
  sortScreenerStocks,
  exportScreenerToCsv,
} from "./screener.ts";

const mockStocks = [
  {
    symbol: "AAPL",
    name: "Apple Inc.",
    sector: "Technology",
    marketCap: 3000, // $3T
    peRatio: 28,
    dividendYield: 0.5,
    beta: 1.2,
    price: 180,
    change: 2,
    changePercent: 1.1,
  },
  {
    symbol: "JPM",
    name: "JPMorgan Chase",
    sector: "Financials",
    marketCap: 500, // $500B
    peRatio: 12,
    dividendYield: 2.5,
    beta: 1.0,
    price: 160,
    change: -1,
    changePercent: -0.6,
  },
  {
    symbol: "TSLA",
    name: "Tesla Inc.",
    sector: "Consumer Cyclical",
    marketCap: 600, // $600B
    peRatio: 65,
    dividendYield: 0.0,
    beta: 1.5,
    price: 220,
    change: 8,
    changePercent: 3.7,
  },
];

test("filterScreenerStocks filters by sector", () => {
  const filtered = filterScreenerStocks(mockStocks, { sector: "Technology" });
  assert.equal(filtered.length, 1);
  assert.equal(filtered[0].symbol, "AAPL");
});

test("filterScreenerStocks filters by market cap categories", () => {
  // Mega: > 100B, Large: 10B - 100B, MidSmall: < 10B
  // In mock stocks, marketCaps are 3000, 500, 600. All are Mega.
  const mega = filterScreenerStocks(mockStocks, { marketCap: "Mega" });
  assert.equal(mega.length, 3);

  const mid = filterScreenerStocks(mockStocks, { marketCap: "MidSmall" });
  assert.equal(mid.length, 0);
});

test("filterScreenerStocks filters by valuation categories", () => {
  // Value: PE < 15, Growth: PE > 30, Income: yield > 2.0%
  const value = filterScreenerStocks(mockStocks, { valuation: "Value" });
  assert.equal(value.length, 1);
  assert.equal(value[0].symbol, "JPM");

  const growth = filterScreenerStocks(mockStocks, { valuation: "Growth" });
  assert.equal(growth.length, 1);
  assert.equal(growth[0].symbol, "TSLA");

  const income = filterScreenerStocks(mockStocks, { valuation: "Income" });
  assert.equal(income.length, 1);
  assert.equal(income[0].symbol, "JPM");
});

test("sortScreenerStocks sorts correctly", () => {
  const capSorted = sortScreenerStocks(mockStocks, "marketCap");
  assert.equal(capSorted[0].symbol, "AAPL"); // 3000
  assert.equal(capSorted[1].symbol, "TSLA"); // 600
  assert.equal(capSorted[2].symbol, "JPM");  // 500

  const changeSorted = sortScreenerStocks(mockStocks, "changePercent");
  assert.equal(changeSorted[0].symbol, "TSLA"); // 3.7%
  assert.equal(changeSorted[1].symbol, "AAPL"); // 1.1%
  assert.equal(changeSorted[2].symbol, "JPM");  // -0.6%

  // Sort with null values
  const stocksWithNulls = [
    { ...mockStocks[0], peRatio: null, dividendYield: null },
    { ...mockStocks[1], peRatio: 15, dividendYield: 2.5 },
    { ...mockStocks[2], peRatio: 30, dividendYield: null },
  ];

  const peSorted = sortScreenerStocks(stocksWithNulls, "peRatio");
  // Expected order: 15 (JPM), 30 (TSLA), null (AAPL)
  assert.equal(peSorted[0].symbol, "JPM");
  assert.equal(peSorted[1].symbol, "TSLA");
  assert.equal(peSorted[2].symbol, "AAPL");

  const yieldSorted = sortScreenerStocks(stocksWithNulls, "dividendYield");
  // Expected order: 2.5 (JPM), null (AAPL), null (TSLA)
  assert.equal(yieldSorted[0].symbol, "JPM");
});

test("filterScreenerStocks marketCap boundary categories", () => {
  const boundaryStocks = [
    { ...mockStocks[0], symbol: "STOCK_A", marketCap: 50 },  // Large
    { ...mockStocks[0], symbol: "STOCK_B", marketCap: 5 },   // MidSmall
    { ...mockStocks[0], symbol: "STOCK_C", marketCap: 150 }, // Mega
  ];

  const mega = filterScreenerStocks(boundaryStocks, { marketCap: "Mega" });
  assert.equal(mega.length, 1);
  assert.equal(mega[0].symbol, "STOCK_C");

  const large = filterScreenerStocks(boundaryStocks, { marketCap: "Large" });
  assert.equal(large.length, 1);
  assert.equal(large[0].symbol, "STOCK_A");

  const midSmall = filterScreenerStocks(boundaryStocks, { marketCap: "MidSmall" });
  assert.equal(midSmall.length, 1);
  assert.equal(midSmall[0].symbol, "STOCK_B");
});

test("sortScreenerStocks handles sorting multiple PE nulls and non-null comparison", () => {
  const stocks = [
    { ...mockStocks[0], symbol: "A", peRatio: null },
    { ...mockStocks[1], symbol: "B", peRatio: null },
    { ...mockStocks[2], symbol: "C", peRatio: 10 },
  ];

  const sorted = sortScreenerStocks(stocks, "peRatio");
  // C (10) should be first, then A and B (nulls)
  assert.equal(sorted[0].symbol, "C");
  assert.equal(sorted[1].peRatio, null);
  assert.equal(sorted[2].peRatio, null);
});

test("sortScreenerStocks keeps all-null PE lists stable and complete", () => {
  const stocks = [
    { ...mockStocks[0], symbol: "A", peRatio: null },
    { ...mockStocks[1], symbol: "B", peRatio: null },
  ];
  const sorted = sortScreenerStocks(stocks, "peRatio");
  assert.equal(sorted.length, 2);
});

test("filterScreenerStocks filters by beta category", () => {
  const lowBeta = filterScreenerStocks(
    [
      { ...mockStocks[0], beta: 0.5 },
      { ...mockStocks[1], beta: 1.0 },
      { ...mockStocks[2], beta: 1.6 },
    ],
    { betaRange: "Low" }
  );
  assert.equal(lowBeta.length, 1);
  assert.equal(lowBeta[0].beta, 0.5);

  const marketBeta = filterScreenerStocks(
    [
      { ...mockStocks[0], beta: 0.5 },
      { ...mockStocks[1], beta: 1.0 },
      { ...mockStocks[2], beta: 1.6 },
    ],
    { betaRange: "Market" }
  );
  assert.equal(marketBeta.length, 1);
  assert.equal(marketBeta[0].beta, 1.0);

  const highBeta = filterScreenerStocks(
    [
      { ...mockStocks[0], beta: 0.5 },
      { ...mockStocks[1], beta: 1.0 },
      { ...mockStocks[2], beta: 1.6 },
    ],
    { betaRange: "High" }
  );
  assert.equal(highBeta.length, 1);
  assert.equal(highBeta[0].beta, 1.6);
});

test("exportScreenerToCsv formats valid CSV with proper headers", () => {
  const csv = exportScreenerToCsv(mockStocks);
  assert.ok(csv.startsWith("Symbol,Name,Sector,Market Cap ($B),Price,Change %,P/E,Div Yield %,Beta"));
  assert.ok(csv.includes("AAPL,Apple Inc.,Technology,3000.00,180.00,1.10,28.0,0.50,1.20"));
});
