...
test("getInsiderTransactions parses transactions feed correctly", async () => {
  ...
  try {
    ...
    // Finnhub returns isDerivative, not isDirectShare. Reading the wrong field
    // name yields !!undefined === false, so assert true to catch that regression.
    assert.equal(list[0].isDerivative, true);
  } finally {
    process.env.FINNHUB_API_KEY = originalKey;
    globalThis.fetch = originalFetch;
  }
...
test("getQuote fallback fields for null change/changePercent", async () => {
  const originalKey = process.env.FINNHUB_API_KEY;
  const originalFetch = globalThis.fetch;
  process.env.FINNHUB_API_KEY = "mock-key";
  globalThis.fetch = async () => {
...
test("searchSymbols fallback when result is missing", async () => {
  const originalKey = process.env.FINNHUB_API_KEY;
  const originalFetch = globalThis.fetch;
  process.env.FINNHUB_API_KEY = "mock-key";
  globalThis.fetch = async () => {
...
test("getProfile fallback fields when profile is empty", async () => {
  const originalKey = process.env.FINNHUB_API_KEY;
  const originalFetch = globalThis.fetch;
  process.env.FINNHUB_API_KEY = "mock-key";
  globalThis.fetch = async () => {
  ...
  try {
    const profile = await getProfile("AAPL");
    assert.equal(profile.name, "AAPL");
    assert.equal(profile.exchange, "");
    assert.equal(profile.industry, "");
    assert.equal(profile.logo, "");
    assert.equal(profile.weburl, "");
    assert.equal(profile.ipo, "");
    assert.equal(profile.marketCap, 0);
    assert.equal(profile.sharesOutstanding, 0);
  } finally {
    process.env.FINNHUB_API_KEY = originalKey;
    globalThis.fetch = originalFetch;
  }
...
test("getKeyMetrics fallback fields when metric object or fields are missing", async () => {
  const originalKey = process.env.FINNHUB_API_KEY;
  const originalFetch = globalThis.fetch;
  process.env.FINNHUB_API_KEY = "mock-key";
  globalThis.fetch = async () => {
  ...
  try {
    const metrics = await getKeyMetrics("AAPL");
    assert.equal(metrics.high52, null);
...
test("getEarningsCalendar fallback when earningsCalendar is missing", async () => {
  const originalKey = process.env.FINNHUB_API_KEY;
  const originalFetch = globalThis.fetch;
  process.env.FINNHUB_API_KEY = "mock-key";
  globalThis.fetch = async () => {
...
test("getInsiderTransactions fallback when data is missing", async () => {
  const originalKey = process.env.FINNHUB_API_KEY;
  const originalFetch = globalThis.fetch;
  process.env.FINNHUB_API_KEY = "mock-key";
  globalThis.fetch = async () => {
...
test("getEarningsSurprises fetches and returns earnings history", async () => {
  const originalKey = process.env.FINNHUB_API_KEY;
  const originalFetch = globalThis.fetch;
  process.env.FINNHUB_API_KEY = "mock-key";
  globalThis.fetch = async (url) => {
    assert.match(url, /finnhub\.io\/api\/v1\/stock\/earnings/);
    return new Response(
      JSON.stringify([
        {
          actual: 1.5,
  ...
  try {
    const list = await getEarningsSurprises("AAPL");
    assert.equal(list.length, 1);
    assert.equal(list[0].actual, 1.5);
    assert.equal(list[0].estimate, 1.4);
    assert.equal(list[0].surprise, 0.1);
  } finally {
    process.env.FINNHUB_API_KEY = originalKey;
    globalThis.fetch = originalFetch;
  }
