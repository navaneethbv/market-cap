...
test("runMonteCarloSimulation handles positive growth distribution", () => {
  ...
  assert.ok(result.probabilityOfSuccess <= 100);
});
...
test("runMonteCarloSimulation clamps negative portfolio values to zero", () => {
  const result = runMonteCarloSimulation({
    initialCapital: 100,
  ...
  // Ensure percentiles stay non-negative due to currentVal < 0 -> 0 clamping
  assert.ok(result.points.every((p) => p.percentile10 >= 0 && p.percentile50 >= 0));
});
