...
test("normalizeCorrelationSymbols uppercases, dedupes, filters, and caps symbols", () => {
  assert.deepEqual(
    normalizeCorrelationSymbols(" aapl, MSFT, aapl, GOOG$$, qqq, spy "),
    ["AAPL", "MSFT", "QQQ", "SPY"]
  );
...
test("normalizeCorrelationSymbols splits on whitespace too", () => {
  assert.deepEqual(
    normalizeCorrelationSymbols("aapl msft\tnvda,googl"),
...
test("calculateDailyReturns aggregates intraday candles to daily closes", () => {
  const candles = [
    { time: "2026-06-01 09:30", open: 100, high: 100, low: 100, close: 101, volume: 100 },
    { time: "2026-06-01 10:30", open: 101, high: 101, low: 101, close: 102, volume: 100 },
    { time: "2026-06-02 09:30", open: 102, high: 102, low: 102, close: 104, volume: 100 },
    { time: "2026-06-02 10:30", open: 104, high: 104, low: 104, close: 103, volume: 100 },
  ];

  const map = calculateDailyReturns(candles);
  // Day 1 close = 102 (last intraday close), day 2 close = 103 (last intraday close)
  assert.equal(map.size, 1);
  assert.equal(map.get("2026-06-02"), (103 - 102) / 102);
});
...
test("calculateCorrelationMatrix sets null self-correlation for short return sizes", () => {
  const candlesShort = Array(10).fill(null).map((_, idx) => ({
    time: `2026-06-${String(idx + 1).padStart(2, "0")}`,
    open: 100 + idx,
    high: 100 + idx,
    low: 100 + idx,
  ...
  const result = calculateCorrelationMatrix([
    { symbol: "AAPL", candles: candlesShort },
  ]);

  assert.equal(result.matrix.AAPL.AAPL, null);
});
