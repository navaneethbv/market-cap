...
test("getUpcomingHolidays includes adjacent-year holidays", () => {
  // Late in 2026 all 2026 holidays have passed; the next one is 2027.
  const events = getUpcomingHolidays("2026-12-28", 2026);
  assert.ok(events.length > 0);
  assert.ok(events[0].name === "New Year's Day");
  assert.ok(events[0].date.startsWith("2027-"));

  // Mid-year views still prefer current-year holidays.
  const midYear = getUpcomingHolidays("2026-02-01", 2026);
  assert.ok(midYear.every((event) => event.date.startsWith("2026-")));
});
