...
test("getAppOrigin handles invalid configured urls", () => {
  delete process.env.APP_URL;
  delete process.env.NEXT_PUBLIC_APP_URL;
  delete process.env.VERCEL_PROJECT_PRODUCTION_URL;
  delete process.env.VERCEL_URL;

  process.env.APP_URL = "http://[::1"; // triggers URL parse exception
  assert.equal(getAppOrigin("https://attacker.example"), "http://localhost:3000");
});

test("getAppOrigin handles non-http/https configured protocols", () => {
  delete process.env.NEXT_PUBLIC_APP_URL;
  delete process.env.VERCEL_PROJECT_PRODUCTION_URL;
  delete process.env.VERCEL_URL;
  process.env.APP_URL = "ftp://ftp.example.com";
  assert.equal(getAppOrigin("http://localhost:3000"), "http://localhost:3000");
});

test("getAppOrigin handles missing request origin", () => {
  delete process.env.APP_URL;
  delete process.env.NEXT_PUBLIC_APP_URL;
  delete process.env.VERCEL_PROJECT_PRODUCTION_URL;
  delete process.env.VERCEL_URL;
  assert.equal(getAppOrigin(null), "http://localhost:3000");
  assert.equal(getAppOrigin(undefined), "http://localhost:3000");
});

test("getAppOrigin handles invalid request origin URLs", () => {
  delete process.env.APP_URL;
  delete process.env.NEXT_PUBLIC_APP_URL;
  delete process.env.VERCEL_PROJECT_PRODUCTION_URL;
  delete process.env.VERCEL_URL;
  assert.equal(getAppOrigin("invalid url with spaces"), "http://localhost:3000");
});
