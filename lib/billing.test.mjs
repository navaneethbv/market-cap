...
test("getBillingState reads current billing state on every check", async () => {
  ...
  try {
    ...
    // 2. A subsequent lookup must not use process-local stale state.
    invalidateBillingCache(userId);
    const currentState = await getBillingState(userId, mockSupabase);
    assert.equal(currentState.isPro, true);
...
test("getBillingState falls back to importing supabase server when no client is provided", async () => {
  const originalKey = process.env.STRIPE_SECRET_KEY;
  process.env.STRIPE_SECRET_KEY = "sk_test_mock_123";
  try {
    const state = await getBillingState("user_no_client");
    assert.deepEqual(state, FREE_BILLING_STATE);
  } finally {
    process.env.STRIPE_SECRET_KEY = originalKey;
  }
...
test("getBillingState handles Stripe deleted customer with no metadata", async () => {
  const originalKey = process.env.STRIPE_SECRET_KEY;
  process.env.STRIPE_SECRET_KEY = "sk_test_mock_123";
  try {
    const stripe = getStripe();
    const userId = "user_test_deleted";

    stripe.customers.retrieve = async (id) => {
      assert.equal(id, "cus_test_deleted");
      return { id: "cus_test_deleted", deleted: true };
    };
    ...
    const mockSupabase = {
      from: () => ({
        select: () => ({
          eq: () => ({
            maybeSingle: async () => {
              return { data: { stripe_customer_id: "cus_test_deleted" } };
            },
          }),
        }),
      }),
