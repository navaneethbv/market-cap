...
test("calculatePortfolioHistory skips holdings with no candle data entirely", () => {
  const holdings = [
    {
      id: "1",
      symbol: "AAPL",
  ...
  // MSFT is missing from candlesMap entirely (its candle fetch failed).
  const candlesMap = {
    AAPL: [
      { time: "2026-06-12", open: 150, high: 152, low: 149, close: 151, volume: 100 },
      { time: "2026-06-25", open: 155, high: 156, low: 154, close: 155, volume: 100 },
  ...
  // At 2026-06-12 AAPL is owned and priced; MSFT is not owned yet.
  assert.equal(history.length, 2);
  assert.equal(history[0].value, 10 * 151);
  assert.equal(history[0].costBasis, 10 * 150);

  // At 2026-06-25 both are owned, but MSFT has no price data. It must be
  // omitted from both value and cost basis, not counted at $0 (which would
  // fabricate a $1500 loss on top of the correct AAPL contribution).
  assert.equal(history[1].value, 10 * 155);
  assert.equal(history[1].costBasis, 10 * 150);
});
